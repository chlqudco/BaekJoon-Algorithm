...
//구간 합 트리 생성
int make_tree2(int s, int e, int node) {
	if (s == e) return tree2[node] = arr[s];
	int mid = (s + e) / 2;
	return tree2[node] = max(make_tree2(s, mid, node * 2), make_tree2(mid + 1, e, node * 2 + 1));
}
...
//최소값 구하기
int get_max_num(int node, int s, int e, int l, int r)
{
	if (e < l || r < s) return -1; // 찾아야하는 구간과 노드구간이 겹치지 않을 때
	if (l <= s && e <= r) return tree2[node]; // 찾아야하는 구간내에 노드구간이 포함될 때
	int mid = (s + e) / 2;
	// 찾아야하는 구간이 노드구간에 포함되거나, 부분적으로 겹치는 경우
	return max(get_max_num(2 * node, s, mid, l, r), get_max_num(2 * node + 1, mid + 1, e, l, r));
}
...
void Calculate() {
	...
	for (int i = 0; i < M; i++){
		cin >> a >> b;
		cout << get_min_num(1, 0, N - 1, a - 1, b - 1) <<" "<< get_max_num(1, 0, N - 1, a - 1, b - 1) << "\n";
	}
}
